...
p = Figure(resolution=(1600,1200),fontsize=20)
# p = Figure()

ax = Axis(p[1,1], xlabel="x",ylabel="y", xlabelsize=30,ylabelsize=30)

sp = streamplot!(ax,B,-0.5..0.5,-0.5..0.5,stepsize=0.001)
cp = contour!(ax,XY,XY,Z,color=:black,linewidth=4.0)

hm = heatmap!(ax,XY,XY,Z,colormap=(:thermal,0.6))

Colorbar(p[1,2],hm,label=L"u(x,y,t_f)",labelsize=30)
